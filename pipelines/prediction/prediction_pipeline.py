...
def prediction():
    import sys
    sys.path.append('/path/your/prediction')  # Adjust the path as needed
...
prediction_component = components.create_component_from_func(
    func=prediction,
    base_image='/path/your/image',  # Adjust the image path as needed
)
...
def prediction_pipeline(
    ...
    dsl.get_pipeline_conf().set_image_pull_secrets(
        [kubernetes.client.V1LocalObjectReference(name="public_aipops")]
    )
    vop = dsl.PipelineVolume(pvc='croffle-pvc')

    mount_path = "/aiplatform/"

    prediction_task = prediction_component().set_cpu_limit(cpu_limit)\
